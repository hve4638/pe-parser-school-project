...
namespace PELog {
    ...
    void PEPrinter::printSectionHeader(const PEStructure& peStructure) {
        for (auto const& element : peStructure.sectionList) {
            tcout << format(_T("Section Name: {:s}"), get<0>(element)) << endl;
            tcout << format(_T("          > VirtualAddress:   0x{:x}"), get<1>(element)) << endl;
            tcout << format(_T("          > PointerToRawData: 0x{:x}"), get<2>(element)) << endl;
            tcout << format(_T("          > SizeOfRawData:    0x{:x}"), get<3>(element)) << endl;
            tcout << format(_T("          > Characteristics:  0x{:x}"), get<4>(element)) << endl;
        }
    };
