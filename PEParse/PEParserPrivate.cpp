...
namespace PEParse {
    ...
    BOOL PEParser::parseDosHeader(void) {
        if (m_peReader->readData(0x0, &m_peStruct.dosHeader, sizeof(IMAGE_DOS_HEADER)) >= 0) {
            printBuffer((BYTE*)&m_peStruct.dosHeader, sizeof(IMAGE_DOS_HEADER));
    ...
    SIZE_T PEParser::updateSectionHeaders(size_t sectionHeaderOffset, WORD numberOfSections) {
        ...
        for (WORD index = 0; index < numberOfSections; index++) {
            if (m_peReader->readData(sectionHeaderOffset, &sectionHeader, sizeof(IMAGE_SECTION_HEADER)) >= 0) {
                m_peStruct.sectionList.push_back(
                    make_tuple(getString((const char*)sectionHeader.Name, IMAGE_SIZEOF_SHORT_NAME),
                        sectionHeader.VirtualAddress, sectionHeader.PointerToRawData, sectionHeader.SizeOfRawData, sectionHeader.Characteristics));
                count++;
            }
