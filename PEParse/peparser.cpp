...
void PEParser::ParseDosHeader() {
	DosHeader = (IMAGE_DOS_HEADER*)m_base;
	if (DosHeader->e_magic != IMAGE_DOS_SIGNATURE) CloseAndAbort("parse fail: DOS_HEADER");
}
void PEParser::ParseNTHeaderAndSetMachineType() {
	m_ntHeader = (IMAGE_NT_HEADERS32*)((BYTE*)m_base + DosHeader->e_lfanew);
